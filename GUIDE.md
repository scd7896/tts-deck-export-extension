...
- `src/background.ts`
  ...
  - CORS로 직접 읽기 어려운 카드 이미지에 대해 background fetch 우회 경로를 제공합니다.
...
- `src/utils/card.ts`
  ...
  - 특정 카드 이미지 URL이 CORS에 막히는 경우 background fetch 우회를 사용합니다.
...
추가로 현재는 아래 우회 기능도 담당합니다.

- `LOAD_CACHED_IMAGE` 메시지를 받으면 이미지 URL을 직접 fetch
- fetch한 `Blob`을 base64 data URL로 변환
- content 쪽에 다시 전달
...
### 카드 이미지 CORS 이슈와 현재 해결 방식

일부 카드 이미지는 아래와 같은 별도 asset 도메인에서 제공됩니다.

- `https://assets.cookierunbraverse.com/...`

이 경우 브라우저에서 `Image()`로 직접 로드한 뒤 canvas에 그리려고 하면
CORS 문제가 발생할 수 있습니다.

원인:

- 페이지 origin은 `cookierunbraverse.com`
- 카드 이미지 origin은 `assets.cookierunbraverse.com`
- 두 origin이 다르기 때문에 cross-origin 이미지로 취급됨
- 서버가 canvas 사용을 위한 CORS 헤더를 충분히 주지 않으면 직접 로드가 실패하거나
  canvas export가 막힘

현재 프로젝트는 이 문제를 아래 방식으로 우회합니다.

1. `src/utils/card.ts`에서 카드 URL을 로드할 때
2. URL hostname이 `assets.cookierunbraverse.com`이면
3. 직접 `Image()` 로드 대신 `chrome.runtime.sendMessage`로 background에 요청
4. `src/background.ts`가 해당 이미지를 fetch
5. fetch한 `Blob`을 base64 data URL로 변환
6. content 쪽이 그 data URL을 다시 `Image` 소스로 사용

왜 data URL을 쓰는가:

- runtime message로 `Blob` 자체를 주고받을 때 직렬화 문제가 발생할 수 있었음
- 이 프로젝트에서는 이미 `Blob -> base64` 경로가 안정적으로 동작한 전례가 있음
- 따라서 현재 우회 방식은 문자열 기반 data URL 전달을 사용함

이 동작을 위해 필요한 manifest 설정:

- `src/manifest.json`의 `host_permissions`
- `https://assets.cookierunbraverse.com/*`

주의:

- `manifest`에 host permission을 추가해도 content script의 직접 CORS 제한이 사라지는 것은 아님
- 현재 방식은 manifest로 background fetch 권한을 확보하고, background를 통해 우회하는 구조임
...
- `src/background.ts`: popup -> content 메시지 브리지 및 카드 이미지 CORS 우회 fetch
...
- `src/utils/card.ts`: 덱 시트 생성기 및 카드 이미지 로드 전략 분기
