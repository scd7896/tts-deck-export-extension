...
Simulator에서 사용할 수 있는 덱 시트 이미지로 합성하고, 팝업에서 미리보기와
다운로드를 제공하는 것이 주된 목적입니다.
...
- `src/popup/popup.ts`
  - popup UI 로직
  - 생성된 이미지를 요청하고, 미리 보여주고, 다운로드를 처리합니다.
...
- `src/utils/drawSimulator.ts`
  ...
  - 현재는 실험용이거나 미래 기능 준비 코드에 가깝습니다.

## 런타임 동작 흐름

### 1. Popup

`src/popup/popup.ts`는 아래 메시지를 보냅니다.

- `EVENT_NAMES.LOAD_IMAGE`

응답이 오면:

- popup 이미지에 결과를 표시하고
- 생성된 시트로부터 `Deck` 객체를 만들고
- 다운로드 버튼을 활성화합니다.

참고:

- `Deck` 객체는 생성되지만, 현재 UI에서 실제로 활용되지는 않습니다.
...
### 5. drawSimulator 사용 목적 불명확

`drawSimulator.ts`는 popup에서 import되어 실제로 실행되지만, 그 결과가 현재 UI에
노출되지는 않습니다. 이 상태는 아래 중 하나일 가능성이 큽니다.

- 미래 기능을 위한 준비 코드
- 실험 후 남아 있는 코드
- 현재 사용자 가치 없이 popup 작업만 늘리는 코드
